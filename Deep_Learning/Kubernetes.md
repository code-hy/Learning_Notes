...
We create a fastapi application that serves the ONNX model for inference.  
   ```bash
cd service
uv init
rm main.py
...
Create the fastapi application   service/app.py
```python
import numpy as np
...
-  Custom pytorch-style preprocessing functions
...
```
then open http://localhost:8080/docs to see ther API documentation.

Create a test script  service/test.py
```python
import requests
